...
ps=con.prepareStatement("SELECT COUNT(*) FROM CARS_INFO WHERE TYPE='Upcoming Model'");
res=ps.executeQuery();
while(res.next()){
	count=res.getInt(1);
}
...
//Printing pages of site
	...
	ps=con.prepareStatement("select * from(select rownum as rn,id,brand,model,img1,type from cars_info)where rn>=? AND type='Upcoming Model'");
	ps.setInt(1, recNo);
	res=ps.executeQuery();
...
//printing page window
		...
		for(int i=1;i<=lastPage;i++){
		    ...
		    out.println("<a id='OtherPage' href='Upcoming.jsp?Page="+i+"'>"+i+"</a>");
	//for
		}
